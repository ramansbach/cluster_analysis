...
To use it as a template for your own project, you will need to follow the instructions at the [bottom of this page](#using-clustering-as-a-template).
...
`clustering/clustering.py`. The code provided in this file is intentionally rather
...
We place the module code in a file called `clustering.py` in directory called
`clustering`. This structure is a bit confusing at first, but it is a simple way
to create a structure where when we type `import clustering as sb` in an
interactive Python session, the classes and functions defined inside of the
`clustering.py` file are available in the `sb` namespace. For this to work, we
...
In the case of the clustering module, the main classes defined at the bottom of
...
Either way, you can create a `clustering/data` folder in which you can
...
(typically, something like our `clustering/tests/test_clustering.py`). Within each
...
have one such test in `clustering/tests/test_clustering.py` called
...
the top-level directory of the repository (e.g. `/Users/arokem/code/clustering`),
and type:
...
will see a message such as:
    ...
    clustering/tests/test_clustering.py .F...
    ...
    clustering/tests/test_clustering.py:49: AssertionError
...
To document `clustering` we use the [sphinx documentation
...
![RTD conf](https://github.com/uwescience/clustering/blob/master/doc/_static/RTD-advanced-conf.png)

 http://clustering.readthedocs.org/en/latest/
...
A `clustering/version.py` contains all of the information needed for the
installation and for setting up the [PyPI
page](https://pypi.python.org/pypi/clustering) for the software. This
...
`clustering` against Python 2.7, Python 3.3 and Python 3.4). It includes steps
that need to be taken before installation of the software. For example,
installation of the software dependencies. For `clustering`, we use the
...
[here](https://github.com/uwescience/clustering/blob/master/scripts/Figure1.ipynb).


### Using `clustering` as a template
...
Under "old repository clone URL" enter: https://github.com/uwescience/clustering
...
`clustering/clustering.py` to be called `smallish/smallish.py`

	git mv clustering smallish
	git mv smallish/clustering.py smallish/smallish.py
	git mv smallish/tests/test_clustering.py smallish/tests/test_smallish.py

Make a commit recording these changes. Something like:

	git commit -a -m"Moved names from `clustering` to `smallish`"

You will probably want to remove all the example data:

	git rm smallish/data/*
	git commit -a -m"Removed example `clustering` data"

Possibly, you will want to add some of your own data in there.

You will want to edit a few more places that still have `clustering` in them. Type
the following to see where all these files are:

	git grep clustering

You can replace `clustering` for `smallish` quickly with:

	git grep -l 'clustering' | xargs sed -i 's/clustering/smallish/g'
...
Edit all the mentions of `clustering` in the `clustering/__init__.py` file, and in
the `clustering/version.py` file as well.
